...
* [BUGFIX] Kafka: mount a writable `emptyDir` at `/opt/kafka/logs` in the Kafka StatefulSet. The Kafka container runs with `readOnlyRootFilesystem: true`, but the `apache/kafka-native` image writes its own log4j operational logs to `/opt/kafka/logs` by default, which previously had no writable volume, causing Kafka to fail to start. #16329
* [BUGFIX]: Fix bug in `ScaledObject` templates when using `kedaAutoscaling.fallback` #15793
